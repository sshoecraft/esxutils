...
#define M_ASN1_STRING_data ASN1_STRING_get0_data
#endif
...
{ char *endpoint;
  const char *s;
//printf("con_com: soap: %p, http_command: %d, endpoints: %s, action: %s\n", soap, http_command, endpoints, action);
...
{ char host[sizeof(soap->host)];
  int port;
  size_t count;
//printf("try_con_com: soap: %p, http_command: %d, endpoint: %s, action: %s\n", soap, http_command, endpoint, action);
  soap->error = SOAP_OK;
